...
// verticalDataScale mirrors VERTICAL_DATA_SCALE in libavpipe/include/avpipe_utils.h:
// vertical_data values are the crop-window centre as a fraction of the scaled
// frame width, with this fixed denominator. 0 is the left edge, 5000 the centre,
// 10000 the right edge.
const verticalDataScale = 10000

// patternLo/patternHi bound the range where the window is fully inside the frame
// for a 9:16 crop: half the crop width is (9/16)/2 = 0.28125 of the height, which
// for a 16:9 source is 0.28125*(9/16) = 0.158 of the width. Outside that the crop
// clamps to the frame edge and stops tracking.
const (
	patternLo = 1600 // 0.1600 - just inside the left clamp
	patternHi = 8400 // 0.8400 - just inside the right clamp
	fullLo    = 1000 // 0.1000 - deliberately inside the left clamp region
	fullHi    = 9990 // 0.9990 - deliberately inside the right clamp region
)

// pingpongPeriod is one centre -> right -> left -> centre cycle, in frames.
// Fixed rather than derived from -frames so the pan speed stays the same
// whatever the clip length; -frames just decides how many cycles you get.
const pingpongPeriod = 60
...
func shape(name string, n, total int) (float64, error) {
	...
	switch name {
	case "pingpong":
		// Triangle wave, phase-shifted a quarter cycle so it starts and ends at
		// centre: 0.5 -> 1 -> 0 -> 0.5, repeating every pingpongPeriod frames.
		// Continuous across the wrap, so the window never jumps - anything that
		// looks like a jump on playback is the crop, not the pattern.
		// Same shape pan_value() uses in libavpipe/test/test_vertical_crop.c.
		p := math.Mod(float64(n)/pingpongPeriod+0.25, 1)
		return 1 - math.Abs(1-2*p), nil
	case "sweep":
		return t, nil
	case "hold":
		return 0.5, nil
...
func cropX(v uint32, scaledWidth, cropWidth int) int {
	centerX := int(uint64(v) * uint64(scaledWidth) / verticalDataScale)
...
func printTable(vals []uint32, scaledW, cropW int) {
	...
	for n, v := range vals {
		...
		fmt.Fprintf(w, "%d\t%d\t%.4f\t%d\t[%d, %d)\n",
			n, v, float64(v)/verticalDataScale, x, x, x+cropW)
	}
	_ = w.Flush()
}
