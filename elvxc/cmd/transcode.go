...
func InitTranscode(cmdRoot *cobra.Command) error {
	...
	cmdTranscode.PersistentFlags().StringP("rc", "", "", "rate control mode. Valid values for libx264 encoder are 'none', 'cbr', and 'vbr'. Valid values for h264_nvenc are 'constqp', 'vbr', 'cbr', 'cbr_ld'.")
	cmdTranscode.PersistentFlags().StringP("tune", "", "", "tune for encoding params. Valid values for h264_nvenc are 'hq'.")
...
func doTranscode(cmd *cobra.Command, args []string) error {
	...
	rateControl := cmd.Flag("rc").Value.String()
	tune := cmd.Flag("tune").Value.String()
