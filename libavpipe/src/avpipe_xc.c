...
#define MAX_FILL_FRAMES             1200 /* Around 20-40 sec (will make configurable if needed) */
    ...
    int use_custom_pts_unwrap)
{
    ...
    decoder_context->video_last_fps_slot = AV_NOPTS_VALUE;
    ...
    xcparams_t *params)
{
    ...
    rc = validate_video_fps(decoder_context, params);
    if (rc != eav_success)
        return rc;
    ...
    if (params->bypass_transcoding && params->video_fps > 0) {
        elv_err("video_fps requires decoded video and cannot be used with bypass transcoding, url=%s",
            params->url);
        return eav_param;
    }
    ...
    if (decoder_context->live_proto == avp_proto_rtmp && params->video_fps > 0)
        encoder_codec_context->framerate = (AVRational) {params->video_fps, 1};
    else
    ...
    if (decoder_context->live_proto == avp_proto_rtmp && params->video_fps > 0)
        encoder_context->stream[index]->avg_frame_rate = (AVRational) {params->video_fps, 1};
    else
...
video_fps_enabled(
    ...
    xcparams_t *params)
{
    return decoder_context->live_proto == avp_proto_rtmp && params->video_fps > 0;
}

/*
 * Limit the amount of frames generated by the fps filter.
 */
static int
check_fps_gap(
    coderctx_t *decoder_context,
    ...
    xcparams_t *params)
{
    ...
    if (!video_fps_enabled(decoder_context, params) ||
        frame->pts == AV_NOPTS_VALUE)
        return eav_success;

    slot = av_rescale_q_rnd(frame->pts,
        decoder_context->stream[stream_index]->time_base,
        (AVRational) {1, params->video_fps},
        (enum AVRounding) (AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX));

    if (decoder_context->video_last_fps_slot != AV_NOPTS_VALUE &&
        slot > decoder_context->video_last_fps_slot) {
        uint64_t slot_distance = (uint64_t) slot -
            (uint64_t) decoder_context->video_last_fps_slot;
        int64_t fill_frames = (int64_t) (slot_distance - 1);

        if (fill_frames > MAX_FILL_FRAMES) {
            elv_err("fps gap exceeds max fill frames, last_slot=%"PRId64
                " slot=%"PRId64" fill_frames=%"PRId64" max_fill_frames=%d url=%s",
                decoder_context->video_last_fps_slot, slot, fill_frames,
                MAX_FILL_FRAMES, params->url);
            return eav_receive_filter_frame;
        }
    }
    ...
    if (decoder_context->video_last_fps_slot == AV_NOPTS_VALUE ||
        slot > decoder_context->video_last_fps_slot)
        decoder_context->video_last_fps_slot = slot;
    ...
    int debug_frame_level)
{
    ...
    while (response >= 0) {
        ...
        ret = check_fps_gap(decoder_context, frame, stream_index, p);
        if (ret != eav_success)
            return ret;
    ...
    xcparams_t *params)
{
    ...
    if (params->video_fps < 0) {
        elv_err("Invalid video_fps video_fps=%d, url=%s",
            params->video_fps, params->url);
        return eav_param;
    }

    if (params->video_fps > 0) {
        if (strncmp(params->url, "rtmp://", 7)) {
            elv_err("Invalid video_fps - requires RTMP input, video_fps=%d, url=%s",
                params->video_fps, params->url);
            return eav_param;
        }
        if (!(params->xc_type & xc_video) || params->bypass_transcoding) {
            elv_err("Invalid video_fps - requires video transcoding, xc_type=%d, url=%s",
                params->xc_type, params->url);
            return eav_param;
        }
        if (params->force_equal_fduration) {
            elv_err("Invalid video_fps - incompatible with force_equal_fduration, url=%s", params->url);
            return eav_param;
        }
    }
    ...
    xcparams_t *params)
{
    ...
    snprintf(buf, sizeof(buf),
        ...
        1, params->video_time_base, params->video_frame_duration_ts, params->video_fps,
        params->rotate,
