    ...
    const char **used)
{
    ...
    int refs_from_sps = h264_refs_from_extradata(codecpar->extradata, codecpar->extradata_size);
    ...
    xcparams_t *params)
{
    ...
    AVCodecParameters *source_codecpar = decoder_context->stream[index]->codecpar;
    int framerate = 0;
    ...
    if (avpipe_h264_profile(params->profile) > 0) {
        ...
        elv_log("H264 profile set from params profile=%s value=%d url=%s",
            params->profile, encoder_codec_context->profile, params->url);
    } else if (source_codecpar->profile > 0) {
        encoder_codec_context->profile = source_codecpar->profile;
        elv_log("H264 profile copied from source profile=%d url=%s",
            encoder_codec_context->profile, params->url);
    } else {
        ...
        elv_log("H264 profile guessed profile=%d bitdepth=%d width=%d height=%d url=%s",
            encoder_codec_context->profile, params->bitdepth,
            encoder_codec_context->width, encoder_codec_context->height, params->url);
    }
    ...
    if (params->level > 0) {
        ...
        elv_log("H264 level set from params level=%d url=%s",
            encoder_codec_context->level, params->url);
    } else if (source_codecpar->level > 0) {
        encoder_codec_context->level = source_codecpar->level;
        elv_log("H264 level copied from source level=%d url=%s",
            encoder_codec_context->level, params->url);
    } else {
        ...
        elv_log("H264 level guessed level=%d profile=%d bitrate=%"PRId64" framerate=%d width=%d height=%d url=%s",
            encoder_codec_context->level, encoder_codec_context->profile,
            encoder_codec_context->bit_rate, framerate,
            encoder_codec_context->width, encoder_codec_context->height, params->url);
    }
    ...
    xcparams_t *params)
{
    ...
    /*
     * Copy source encoding parameters - critical for partial DASH segment generation
     */
    if (params->h264_refs > 0 || !strcmp(params->format, "dash")) {
        ...
        int refs = source_h264_refs(decoder_context, index, &sps_refs, &codec_context_refs, &used);
    ...
    xcparams_t *params)
{
    ...
    snprintf(buf, sizeof(buf),
        ...
        params->profile ? params->profile : "", params->level, params->h264_refs, params->deinterlace,
