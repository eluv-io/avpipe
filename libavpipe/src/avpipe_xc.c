    ...
    xcparams_t *params)
{
    ...
    // Skipping until the start pts is explicitly possible because we assume there are no B frames
    // involved. This validates that assumption, and warns if it is broken.
    is_b_frame = frame->pict_type == AV_PICTURE_TYPE_B;
    if (is_b_frame && should_attempt_skip_until_start_time_pts(decoder_context, params)) {
        elv_warn("Found unexpected B frame in stream after skipping frames url=%s pts=%"PRId64" dts=%"PRId64,
            params->url, frame->pts, frame->pkt_dts);
    }

    if (ret = check_pts_wrapped(&decoder_context->video_last_input_pts, frame, stream_index))
        return ret;
    ...
    int debug_frame_level)
{
    ...
    while (response >= 0) {
        ...
        ret = validate_video_frame(decoder_context, frame, stream_index, p);
        if (ret == eav_pts_wrapped) {
            av_frame_unref(frame);
            return ret;
        }
...
skip_until_start_time_pts(
    ...
    xcparams_t *params)
{
    if (!should_attempt_skip_until_start_time_pts(decoder_context, params))
        return 0;
...
static int should_attempt_skip_until_start_time_pts(
    coderctx_t *decoder_context,
    xcparams_t *params)
{
