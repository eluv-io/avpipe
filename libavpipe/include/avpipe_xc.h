...
typedef enum avpipe_error_t {
    ...
    eav_receive_frame =
        18, // Error in receiving frame from decoder or audio fifo
    eav_receive_filter_frame =
        19,                  // Error in receiving frame from filter buffer sink
    eav_receive_packet = 20, // Error in receiving packet from encoder
    eav_write_frame =
        21, // Error in writing frame to output stream or audio fifo
...
typedef enum avp_stat_t {
    ...
    in_stat_audio_frame_read =
        2, // # of audio frames read from the input stream
    in_stat_video_frame_read =
        3, // # of video frames read from the input stream
    in_stat_decoding_audio_start_pts =
        4, // PTS of first audio packet went to the decoder
    in_stat_decoding_video_start_pts =
        5,                      // PTS of first video packet went to the decoder
    ...
    out_stat_encoding_end_pts =
        9, // The last PTS encoded. This stat is recorded when a file is closed
    out_stat_start_file =
        10, // Sent when a new file is opened and reports the segment index
    out_stat_end_file =
        11, // Sent when a file is closed and reports the segment index
...
typedef enum avp_container_t {
    ...
    avp_container_mpegts =
        1, // MPEGTS container - can be encapsulated in MPEGTS, SRT, RTP
...
/*
io_mux_ctx_t is used for handling input streams for muxing. It assumes that all
input streams, as ordered by the muxing spec, have the ordering video ->
audio(s) -> caption(s). It stores an input context for each of the input
streams, as well as the number of each.
*/
...
typedef int (*avpipe_opener_f)(const char *url, ioctx_t *ioctx);

typedef int (*avpipe_closer_f)(ioctx_t *ioctx);

typedef int (*avpipe_reader_f)(void *opaque, uint8_t *buf, int buf_size);

typedef int (*avpipe_writer_f)(void *opaque, uint8_t *buf, int buf_size);

typedef int64_t (*avpipe_seeker_f)(void *opaque, int64_t offset, int whence);

typedef int (*avpipe_stater_f)(
    void *opaque, int stream_index, /* The stream_index is not valid for input
                                       stat in_stat_bytes_read. */
 ...
 *
 * The caller specifies which source media audio streams to encode, using
 * xc_params->audio_index, eg.
 *   - audio_index[0] = 1;
 *   - audio_index[1] = 3;
 *   - audio_index[2] = 4;
 ...
 *
 * - decoder
 *   - the audio_stream_index array stores the selected stream index values the
 * same way as xc_params
 ...
 *
 * - encoder
 *   - if the encoding operation is audio join, merge or pan (which effectively
 * takes multiple input steams and makes one output stream)
 *      - audio_stream_index[0] = 0; (output stream index is considered 0 and
 * nb_audio_output is 1)
 *   - otherwise it uses a strange convention (needs fixed - this is impossible
 * to traverse)
 ...
 *   - this array is contiguous and has 'n_audio' elements eg. for the xc_params
 * above
 ...
 *
 * - decoder
 *   - the codec_context array is indexed using the source media stream index
 * values, eg. for the xc_params above
 *     - codec_context[0]  video  (if the source has video on stream_index 0,
 * for example)
 *     - codec_context[1]  audio stream index 1
 *     - codec_context[2]  audio stream index 2 (not selected, per
 * xc_params->audio_index)
 ...
 *   - otherwise the array is indexed the same way as the encoder
 * 'audio_stream_index' array, eg.
...
typedef struct coderctx_t {
    AVFormatContext *format_context; /* Input format context or video output
                                        format context */
    AVFormatContext
        *format_context2[MAX_STREAMS]; /* Audio output format context, indexed
                                          by audio index */
    char filename2[MAX_STREAMS]
                  [MAX_AVFILENAME_LEN]; /* Audio filename formats */
    ...
    AVCodecContext *codec_context[MAX_STREAMS]; /* Audio/video AVCodecContext,
                                                   indexed by stream_index */
    ...
    int64_t video_last_pts_sent_encode; /* Video last pts to encode if tx_type &
                                           tx_video */
    int64_t audio_last_pts_sent_encode[MAX_STREAMS]; /* Audio last pts to encode
                                                        if tx_type & tx_audio */
    int64_t video_last_pts_encoded; /* Video last input pts encoded if tx_type &
                                       tx_video */
    int64_t audio_last_pts_encoded[MAX_STREAMS]; /* Audio last input pts encoded
                                                    if tx_type & tx_audio */

    int64_t
        audio_output_pts; /* Used to set PTS directly when using audio FIFO */
    ...
    int64_t audio_frames_written[MAX_STREAMS]; /* Total audio frames written so
                                                  far */
    int64_t video_pts;                         /* Video decoder/encoder pts */
    int64_t audio_pts[MAX_STREAMS]; /* Audio decoder/encoder pts for each
                                       track/stream */
    int64_t video_input_start_pts; /* In case video input stream starts at PTS >
                                      0 */
    int video_input_start_pts_notified;         /* Will be set as soon as
                                                   out_stat_decoding_video_start_pts is
                                                   fired */
    int64_t audio_input_start_pts[MAX_STREAMS]; /* In case audio input stream
                                                   starts at PTS > 0 */
    int audio_input_start_pts_notified;         /* Will be set as soon as
                                                   out_stat_decoding_audio_start_pts is
                                                   fired */
    int64_t first_decoding_video_pts; /* PTS of first video frame read from the
                                         decoder */
    int64_t first_decoding_audio_pts[MAX_STREAMS]; /* PTS of first audio frame
                                                      read from the decoder */
    int64_t first_encoding_video_pts; /* PTS of first video frame sent to the
                                         encoder */
    int64_t first_encoding_audio_pts[MAX_STREAMS]; /* PTS of first audio frame
                                                      sent to the encoder */
    int64_t
        first_read_packet_pts[MAX_STREAMS]; /* PTS of first packet read - which
                                               might not be decodable */

    int64_t
        video_encoder_prev_pts; /* Previous pts for video output (encoder) */
    int64_t video_duration;     /* Duration/pts of original frame */
    int64_t audio_duration; /* Audio duration/pts of original frame when tx_type
                               == tx_all */
    int64_t
        first_key_frame_pts; /* First video key frame pts, used to synchronize
                                audio and video in UDP live streams */
    int pts_residue;         /* Residue of pts lost in output */

    avp_live_proto_t
        live_proto; /* Live source protocol: MPEGTS, RTMP, SRT, RTP */
    avp_container_t
        live_container; /* Supported live source containers MPEGTS and FLV */

    int is_av_synced;   /* will be set to 1 if audio and video are synced */
    int frame_duration; /* Will be > 0 if parameter set_equal_fduration is set
                           and doing mez making */
    int calculated_frame_duration; /* Approximate/real frame duration of video
                                      stream, will be used to fill video frames
                                    */
...
typedef enum dif_type {
    ...
    dif_bwdif =
        1, // Use filter bwdif mode 'send_field' (two frames per input frame)
    dif_bwdif_frame =
        2 // Use filter bwdif mode 'send_frame' (one frame per input frame)
} dif_type;
...
// Notes:
//   * rc_max_rate and rc_buffer_size must be set together or not at all; they
//   correspond to ffmpeg's bufsize and maxrate
...
typedef struct xcparams_t {
    ...
    int bypass_transcoding; // if 0 means do transcoding, otherwise bypass
                            // transcoding (only copy)
    ...
    int64_t duration_ts;   // Transcode time period [-1 for entire source length
                           // from start_time_ts]
    char *start_segment_str; // Specify index of the first segment  TODO: change
                             // type to int
    int video_bitrate;
    int audio_bitrate;
    ...
    int rc_max_rate;    // Maximum encoding bit rate, used in conjuction with
                        // rc_buffer_size [Default: 0]
    int rc_buffer_size; // Determines the interval used to limit bit rate
                        // [Default: 0]
    int64_t audio_seg_duration_ts; // In ts units. It is used for transcoding
                                   // and producing audio ABR/mez segments
    int64_t video_seg_duration_ts; // In ts units. It is used for transcoding
                                   // and producing video ABR/mez segments
    char *seg_duration; // In sec units. It is used for transcoding and
                        // producing mp4 segments
    int seg_duration_fr;
    ...
    char *crypt_key_url; // Specify a key URL in the manifest [Optional,
                         // Default: key.bin]
    int skip_decoding;   // If set, then skip the packets until start_time_ts
                         // without decoding

    crypt_scheme_t crypt_scheme; // Content protection / DRM / encryption
                                 // [Optional, Default: crypt_none]
    ...
    int seekable; // Default: 0 means not seekable. A non seekable stream with
                  // moov box in
    ...
    int audio_index[MAX_STREAMS]; // Audio index(s) for mez making, may need to
                                  // become an array of indexes
    ...
    char *mux_spec;
    int64_t
        extract_image_interval_ts; // Write frames at this interval. Default: -1
                                   // (will use DEFAULT_FRAME_INTERVAL_S)
    int64_t *extract_images_ts;    // Write frames at these timestamps. Mutually
                                   // exclusive with extract_image_interval_ts
    ...
    int video_frame_duration_ts; // Frame duration of the output video in time
                                 // base

    int debug_frame_level;
    int connection_timeout; // Connection timeout in sec for RTMP or MPEGTS
                            // protocols
...
typedef struct stream_info_t {
    ...
    int stream_id;    // Format-specific stream ID, set by libavformat during
                      // decoding
...
typedef struct cp_ctx_t {
    ...
    // stream_start_pts is necessary information for getting segment splitting
    // to work correctly. We want to preserve the PTS of the output packets when
    // they are written, but segment.c does the splitting assuming that the PTS
    // of the packets starts at 0. By setting the "initial_offset" option, the
    // packets will be offset by that value, but we need to then reduce the PTS
    // of every packet seen by the same amount.
...
typedef struct xctx_t {
    ...
    /*
     * Data structures that are needed for muxing multiple inputs to generate
     * one output. Each video/audio/caption input stream can have multiple input
     * files/parts. Each video/audio/caption input stream has its own coderctx_t
     * and ioctx_t.
     */
    io_mux_ctx_t *in_mux_ctx;             // Input muxer context
    coderctx_t in_muxer_ctx[MAX_STREAMS]; // Video, audio, captions coder input
                                          // muxer context (one video, multiple
                                          // audio/caption)
    ioctx_t
        *inctx_muxer[MAX_STREAMS]; // Video, audio, captions io muxer context
                                   // (one video, multiple audio/caption)
...
/**
 * @brief   Allocates and initializes a xctx_t (transcoder context) for
 * pipelining the input stream. in_handlers, out_handlers, and params ownership
 * is always on the caller, and will never be freed or modified by this
 * function. In case of failure xctx is NULL.
 *
 * @param   xctx            Pointer that will be filled with a partially
 * initialized transcoding context.
 * @param   in_handlers     A pointer to input handlers. Must be properly set up
 * by the application.
 * @param   out_handlers    A pointer to output handlers. Must be properly set
 * up by the application.
 * @param   params          A pointer to the parameters for transcoding.
 *
 * @return  Returns 0 if the initialization of an avpipe xctx_t is successful,
 * otherwise returns corresponding eav error.
 */
int avpipe_init(xctx_t **xctx, avpipe_io_handler_t *in_handlers,
                avpipe_io_handler_t *out_handlers, xcparams_t *params);

/**
 * @brief   Frees the memory and other resources allocated by ffmpeg.
 *
 * @param   xctx       A pointer to the trascoding context that would be
 * destructed.
 * @return  Returns 0.
 */
int avpipe_fini(xctx_t **xctx);
 ...
 *
 * @return  Returns channel layout name if it can find channel layout with
 * corresponding layout id, otherwise empty string.
 */
const char *avpipe_channel_name(int nb_channels, int channel_layout);
 ...
 * @param   xcprobe         A pointer to the xcprobe_t that could contain
 * probing info.
 * @param   n_streams       Will contail number of streams that are probed if
 * successful.
 * @return  Returns 0 if successful, otherwise corresponding eav error.
 */
int avpipe_probe(avpipe_io_handler_t *in_handlers, xcparams_t *params,
                 xcprobe_t **xcprobe, int *n_streams);
 ...
 * @return  Returns 0 if successful, otherwise corresponding eav error.
 */
int avpipe_probe_free(xcprobe_t *xcprobe, int n_streams);

/**
 * @brief   Starts transcoding. Multiple transcoding operations on the same
 * transcoding context is UB. In case of failure avpipe_fini() should be called
 * to avoid resource leak.
 *
 * @param   xctx                A pointer to transcoding context.
 * @param   do_intrument        If 0 there will be no instrumentation, otherwise
 * it does some instrumentation for some ffmpeg functions.
 * @return  Returns 0 if transcoding is successful, otherwise -1.
 */
int avpipe_xc(xctx_t *xctx, int do_instrument);
 ...
 */
int avpipe_init_muxer(xctx_t **xctx, avpipe_io_handler_t *in_handlers,
                      io_mux_ctx_t *in_mux_ctx,
                      avpipe_io_handler_t *out_handlers, xcparams_t *params);

/**
 * @brief   Frees the memory and other resources allocated by avpipe
 * muxer/ffmpeg.
 *
 * @param   xctx        A pointer to the trascoding context that would be
 * destructed.
 * @return  Returns 0.
 */
int avpipe_mux_fini(xctx_t **xctx);
 ...
 */
void init_extract_images(xcparams_t *params, int size);
 ...
 */
void set_extract_images(xcparams_t *params, int index, int64_t value);
 ...
 */
int avpipe_h264_guess_level(int profile_idc, int64_t bitrate, int framerate,
 ...
 */
int avpipe_h264_guess_profile(int bitdepth, int width, int height);
 ...
 *          Returns 0 if profile name is NULL. For invalid profile name return
 * -1.
 */
int avpipe_h264_profile(char *profile_name);
 ...
 * -1.
 */
int avpipe_h265_profile(char *profile_name);

/**
 * @brief   Helper function to obtain FFmpeg constant for an nvidia h264 profile
 * name.
 *
 * @param   profile_name  A pointer to the profile name.
 * @return  Returns the FFmpeg constant if profile name is valid.
 ...
 * -1.
 */
int avpipe_nvh264_profile(char *profile_name);

/**
 * @brief   Helper function to check level.
 *
 * @param   level
 * @return  Returns 1 if the level is valid, otherwise return -1 for an invalid
 * level.
 */
int avpipe_check_level(int level);

/**
 * @brief   Helper function to deep copy an xc_params. In the case of OOM, may
 * fail to initialize all fields.
 *
 * @param   p  A pointer to the transcoding parameters to copy.
 */
xcparams_t *avpipe_copy_xcparams(xcparams_t *p);
